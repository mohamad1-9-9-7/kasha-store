...
export default function ProductDetails() {
  ...
  const handleSubmitRating = async () => {
    ...
    if (!user) { alert(lang === "ar" ? "سجّل الدخول أولاً لتقييم المنتج" : "Please sign in to rate this product"); return; }
    try {
      ...
      setSubmitted(true);
      alert(lang === "ar" ? "✅ شكراً! تقييمك قيد المراجعة وسيظهر بعد موافقة الإدارة." : "✅ Thanks! Your review is pending approval.");
    } catch (e) {
      alert((lang === "ar" ? "❌ فشل إرسال التقييم: " : "❌ Failed to submit review: ") + (e.message || ""));
    } finally {
      ...
      <main style={{ maxWidth: 1060, margin: "32px auto", padding: "0 20px" }}>
          ...
          <div className="pd-body" style={{ padding: "32px", display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {stock !== null && (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: stock === 0 ? "#EF4444" : stock <= (product.lowStockThreshold || 5) ? "#F59E0B" : "#10B981" }}>
                  {stock === 0 ? t("prod_sold_out") : stock <= (product.lowStockThreshold || 5) ? `${t("prod_low")} ${stock} ${t("prod_only")}` : t("prod_in_stock")}
                </span>
              </div>
            )}
            ...
            {timer && (
              <div style={{ background: "#FDF2F8", border: "1px solid #FBCFE8", color: "#DB2777", borderRadius: 12, padding: "10px 16px", fontWeight: 700, fontSize: 14 }}>
                {t("prod_sale_ends")} <strong>{timer}</strong>
              </div>
            )}
            ...
            {canBuy && (
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <span style={{ fontWeight: 700, color: "#334155", fontSize: 14 }}>{t("prod_qty")}</span>
            ...
            <div className="pd-btns" style={{ display: "flex", gap: 10, marginTop: 4 }}>
              <button onClick={handleAdd} disabled={!canBuy || !allVariantsSelected}
                ...
                {added ? t("prod_added") : !canBuy ? t("prod_unavailable") : !allVariantsSelected ? (lang==="ar" ? "اختر الخيارات أولاً" : "Select options first") : t("prod_add")}
              </button>
            ...
            <div style={{ display: "flex", gap: 8, paddingTop: 8, borderTop: "1px solid #F1F5F9" }}>
              <span style={{ fontSize: 13, color: "#94A3B8", fontWeight: 600, alignSelf: "center" }}>{t("prod_share")}</span>
        ...
        <div style={{ background: "#fff", borderRadius: 24, border: "1px solid #F1F5F9", boxShadow: shadow.sm, padding: "28px", marginTop: 24, animation: "fadeUp .5s .1s ease both" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22, flexWrap: "wrap", gap: 12 }}>
            <h2 style={{ fontWeight: 900, fontSize: 18, color: "#0F172A", margin: 0 }}>{t("ratings_title")}</h2>
            {ratings.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#FFFBEB", borderRadius: 14, padding: "8px 16px" }}>
                ...
                <span style={{ color: "#94A3B8", fontSize: 13 }}>{t("ratings_avg")} {ratings.length} {t("ratings_count")}</span>
              </div>
            )}
          </div>
          ...
          {user && !submitted && (
            <div style={{ background: "#F8FAFC", borderRadius: 16, padding: "20px", marginBottom: 24, border: "1.5px dashed #C7D2FE" }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: "#334155", marginBottom: 12 }}>{t("rate_me")}</div>
              <Stars value={myStars} size={28} onChange={setMyStars} />
              <textarea placeholder={t("rate_comment_ph")} value={myComment} onChange={e => setMyComment(e.target.value)} rows={3}
              ...
              <button onClick={handleSubmitRating} disabled={!myStars || submittingRating}
                ...
                {submittingRating ? (lang === "ar" ? "⏳ جاري الإرسال..." : "⏳ Sending...") : t("rate_submit")}
              </button>
            </div>
          )}
          ...
          {ratings.length === 0 ? (
            <div style={{ textAlign: "center", padding: "32px", color: "#94A3B8", background: "#F8FAFC", borderRadius: 14, border: "2px dashed #E2E8F0" }}>
              ...
              <div style={{ fontWeight: 700 }}>{t("ratings_empty")}</div>
            </div>
          ) : (
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 32, animation: "fadeUp .5s .2s ease both" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
              <h2 style={{ fontWeight: 900, fontSize: 20, color: "#0F172A", margin: 0 }}>{t("related_title")}</h2>
              {backCat && <Link to={`/category/${backCat}`} style={{ color: "#6366F1", fontWeight: 700, fontSize: 14 }}>{t("view_all")}</Link>}
            </div>
